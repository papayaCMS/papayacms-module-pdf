...
* @subpackage Free-PDF
* @version $Id: papaya_pdf_options.php 39775 2014-05-02 09:46:06Z weinert $
*/
...
class papaya_pdf_options {
  ...
  *
  * @param papaya_pdf_document $owner
  * @param DOMElement $data
  * @access public
  */
  function initialize($owner, DOMElement $data) {
    $this->owner = $owner;
    $xpath = new DOMXpath($data->ownerDocument);
    foreach ($xpath->evaluate('./*', $data) as $childNode) {
  ...
  * @return void
  */
  function addTags(DOMElement $data) {
    $xpath = new DOMXpath($data->ownerDocument);
    foreach ($xpath->evaluate('tag', $data) as $childNode) {
  ...
  * @return void
  */
  function addTag(DOMElement $data) {
    if ($data->hasAttribute('name')) {
  ...
  function &getPage($pageName) {
    if (isset($pageName) && isset($this->pages[$pageName])) {
      ...
      throw new UnexpectedValueException(
        sprintf(
          'Invalid page definition: "%s".', $pageName
        )
      );
    }
    return $page;
