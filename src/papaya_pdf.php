...
* @subpackage Free-PDF
* @version $Id: papaya_pdf.php 39732 2014-04-08 15:34:45Z weinert $
*/
...
class papaya_pdf extends UFPDF {
  ...
  */
   ...
   * @var string
   */
  public $CurOrientation = 'P';
  /**
  * constructor
  *
  * @param papaya_pdf_document $doc
  ...
  */
  ...
  function addFont($family, $style ='', $fileName = '') {
    ...
    if (file_exists($fileName) && is_file($fileName) && is_readable($fileName)) {
      ...
      $this->Error('Font file not found: '.$fileName);
    }
  }
  ...
  * @param array|DOMNamedNodeMap $attr
  * @return void
  */
  ...
  */
   ...
   */
  function writeBufferLine(
    &$lineBuffer, $para, $lineLength, $align, $spaceCount, $lastLine = FALSE
  ) {
  ...
  function writeListBullet($paraStyle, $para) {
    if ($this->getListIndent($paraStyle) > 0) {
      ...
      if (!$this->inOrderedList) {
        $this->setFont($fontToUse, '', $this->FontSizePt);
      } else {
      ...
      if (!$this->inOrderedList) {
        $this->setFont($font['family'], $font['style'], $font['sizePt']);
      }
    }
  }
  ...
  */
   ...
   *
   * @param string $fileName
   * @param DOMNamedNodeMap|array $attr
   * @access public
   */
  ...
  * @param DOMNamedNodeMap|array $var
  * @return array|NULL
  */
  public static function simplify($var) {
    if (is_array($var)) {
      return $var;
  ...
  */
  function addImportBookmarks($node, $indexList) {
    ...
    foreach ($xpath->evaluate('bookmark', $node) as $childNode) {
