...
* @subpackage Free-PDF
* @version $Id: filter_pdf.php 39818 2014-05-13 13:15:13Z weinert $
*/
...
class filter_pdf extends base_outputfilter {
  ...
  */
  function parsePage($topic, $layout) {
    ...
    $output = papaya_strings::ensureUTF8(
      str_replace($this->badStrings, $this->badStringRepl, $layout->getOutput())
    );

    //dump the transformed xml
    if (isset($_GET['dump']) && $this->papaya()->administrationUser->isLoggedIn()) {
  ...
  */
  function parseBox($topic, $box, $xmlString) {
    ...
    $xsl->parameters()->assign(
      array(
        'PAGE_TODAY' => date('Y-m-d H:i:s'),
        'PAPAYA_WEBSITE_REVISION' => $options->get('PAPAYA_WEBSITE_REVISION', ''),
        'PAGE_THEME' => $themeHandler->getTheme(),
        'PAGE_THEME_SET' => $themeHandler->getThemeSet(),
        'PAGE_THEME_PATH' => $themeHandler->getUrl(),
        'PAGE_THEME_PATH_LOCAL' => $themeHandler->getLocalThemePath(),
        'PAGE_WEB_PATH' => $options->get('PAPAYA_PATH_WEB', '/'),
        'PAGE_LANGUAGE' => $this->papaya()->request->language->code,
        'PAGE_URL_LEVEL_SEPARATOR' => $options->get('PAPAYA_URL_LEVEL_SEPARATOR', ''),
        'PAPAYA_DBG_DEVMODE' => $options->get('PAPAYA_DBG_DEVMODE', FALSE),
    ...
    if (!($result = $xsl->parse(PapayaTemplate::STRIP_ALL))) {
      return FALSE;
    } else {
      return $result;
    }
  ...
  */
  function getConvertedFile($localFileName, $formatSrc) {
    ...
    if (!isset($tmpPath)) {
      ...
      $options = $this->papaya()->plugins->options[$this->guid];
      $tmpPath = $options->get('TMP_PATH', '/tmp/');
  ...
  */
  function getMediaImage($imgAttr, $matchData) {
    ...
    case 'thumb':
      $path = $this->papaya()->options->get('PAPAYA_PATH_THUMBFILES', '');
      if (file_exists($path.$localFile)) {
  ...
  function getLinkForPdf($linkAttr) {
    if (!empty($linkAttr['href'])) {
      if (substr($linkAttr['href'], 0, 1) != '#') {
        if (PapayaFilterFactory::isUrl($linkAttr['href'], TRUE)) {
