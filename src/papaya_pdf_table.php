...
* @subpackage Free-PDF
* @version $Id: papaya_pdf_table.php 39522 2014-03-05 19:42:35Z weinert $
*/
  ...
  */
  function setColumnWidth($colIdx, $width) {
    if (isset($this->_cols[$colIdx])) {
      /** @var papaya_pdf_table_column $column */
      $column = $this->_cols[$colIdx];
   ...
   * @param float|int $max needed max width, default value 0
   * @access public
   */
  ...
  */
  function calcColumnWidths($maxWidth) {
    ...
    foreach ($this->_cols as $column) {
      $tableMinWidth += $column->getMinWidth();
      $tableWidthDiff += $column->getWidthDiff();
    }
    if ($maxWidth > $tableMinWidth) {
      ...
      /** @var papaya_pdf_table_column $column */
      foreach ($this->_cols as $column) {
        $column->calcWidth($widthOverhead, $tableWidthDiff);
